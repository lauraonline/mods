...
SMODS.Joker {
  ...
  loc_txt = {
    ...
    text = {
      ...
      "(Currently at {C:chips}#1#{} Chips)",
      "{s:0.8,C:mult}We fuse. We merge. We grow.{}",
      "{s:0.8,C:inactive}(Art made by Tac!){}"
    }
  },
  config = {extra = {chips = 0, chips_gain = 1, odds = 12}},
  loc_vars = function(self, info_queue, card)
    return {vars = {card.ability.extra.chips, ''..(G.GAME and G.GAME.probabilities.normal or 1), card.ability.extra.chips_gain, card.ability.extra.odds}}
  end,
  rarity = 2,
  unlocked = true,
  discovered = true,
  ...
  calculate = function(self, card, context)
    if context.individual and context.cardarea == G.play then
      if (context.other_card:get_id() >= -100000) then
        card.ability.extra.chips = card.ability.extra.chips + card.ability.extra.chips_gain
          return{
    ...
    if context.after and not context.repetition and not context.blueprint then
      if pseudorandom('crumbly') < G.GAME.probabilities.normal/card.ability.extra.odds then
        G.E_MANAGER:add_event(Event({
          func = function()
            play_sound('tarot1')
            card.T.r = -0.2
...
SMODS.Joker {
  ...
  loc_txt = {
    ...
    text = {
      "{C:mult}+#1#{} Mult and {C:chips}+#2#{} Chips",
      "{C:mult}-#3#{} Mult and {C:chips}-#3#{} Chips for every",
      "{C:attention}hand{} played"
    }
  },
  config = {extra = {mult = 10, chips = 10, loss = 1}},
  loc_vars = function(self, info_queue, card)
  ...
  if card.ability.extra.mult - card.ability.extra.loss <= 0 then 
    G.E_MANAGER:add_event(Event({
        func = function()
            play_sound('tarot1')
            card.T.r = -0.2
